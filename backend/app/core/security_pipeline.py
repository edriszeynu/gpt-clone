...
class InputSanitizer:
    ...
    INJECTION_PATTERNS = [
        ...
        r"pretend\s+you\s+are\s+(a\s+)?(different|new|another|evil|uncensored)",
        r"act\s+as\s+(if\s+)?you\s+(are|were|have\s+no)",
        r"bypass\s+(all\s+)?restrictions",
        r"reveal\s+(your|the)\s+(system|hidden)\s+(instructions|prompt)",
        r"you\s+are\s+now\s+(DAN|jailbroken|uncensored|evil)",
...
class OutputValidator:
    ...
    HARMFUL_PATTERNS = [
        re.compile(r"here('s| is) (how|the way) to (hack|steal|exploit|attack)", re.I),
